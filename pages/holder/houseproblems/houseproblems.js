...
Page(observer({
  ...
  getList(cb) {
    ...
    fetch({
      Act: 'HCGetHouseCheckForDForListType',
      Data: JSON.stringify({
  ...
  getDetail(e) {
    ...
    wx.navigateTo({
      url: `/pages/holder/shproblemlist/shproblemlist?id=${id}&status=${status}&houseno=${houseno}&building=${this.data.building}&unit=${this.data.unit}`
    })
  },
  onLoad(options) {
